...
public class MDNSDiscovery implements LXIDiscovery {
	...
	@Override
	public Set<MDNSInstrumentEndpoint> discover(NetworkInterface networkInterface) throws IOException {
		try(JmDNS jmdns = JmDNS.create(networkInterface.getInterfaceAddresses().get(0).getAddress())) {
			Set<MDNSInstrumentEndpoint> devices = new HashSet<>();
	...
	public static class MDNSInstrumentEndpoint extends LXIDiscovery.InstrumentEndpoint {

		public MDNSInstrumentEndpoint(InetAddress host, int port, DeviceIdentifier deviceIdentifier) {
